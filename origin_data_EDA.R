...
Reg_XY_data <- read_csv("C:/Users/frank/OneDrive/桌面/Reg_XY_data.csv")
View(Reg_XY_data)
...
basic_eda_origin <- function(Reg_XY_data)
{glimpse(Reg_XY_data)
  print(Reg_XY_data)
  print(profiling_num(Reg_XY_data))
  plot_num(Reg_XY_data)
  describe(Reg_XY_data)}

basic_eda_origin(Reg_XY_data)
...
#[盒形圖 box plot] 
#<法一>
boxplot(Reg_XY_data$X_TSM, #指定資料
        ...
        plot = TRUE)
boxplot(Reg_XY_data$Y_2330, #指定資料
        col = "steelblue", #指定圖表顏色
        outline = TRUE, #開啟outline
        ...
        plot = TRUE)
#<法二> 
ggplot(Reg_XY_data, aes(x = X_TSM, y = 1)) +
  geom_boxplot()

ggplot(Reg_XY_data, aes(x = 1, y = Y_2330)) +
  geom_boxplot()+
  coord_flip() #反轉
...
#<法一>
ggplot(Reg_XY_data, aes(X_TSM)) +
  geom_histogram(bins=10)

#<法二>
hist(Reg_XY_data$X_TSM , #指定繪圖對象
...
stem(Reg_XY_data$X_TSM)

# [折線圖 line plot]
line_plot_X <- ggplot(Reg_XY_data, aes(x = Date, y = X_TSM)) +
  geom_line()
line_plot_X

line_plot_Y <- ggplot(Reg_XY_data, aes(x = Date, y = Y_2330)) +
  geom_line()
line_plot_Y
#=========================沒成功================================
# [雙y軸折線圖]
...
#[散佈圖 scatter plot]
#<法一>
scatter_plot <- ggplot(Reg_XY_data, aes(x = X_TSM, y = Y_2330)) +
  geom_point()
scatter_plot

#<法二>
ggplot(Reg_XY_data, aes(x = X_TSM, y = Y_2330)) +
  ...
  #[qq-plot]
  #<法一>
  p <- ggplot(Reg_XY_data , aes(sample=X_TSM))+
  geom_qq() + geom_qq_line()
ggplotly(p)

#<法二>
qqnorm(Reg_XY_data$X_TSM, main = "台積電ADR股價報酬率 Q-Q Plot",
       xlab = "Theoretical Quantiles", ylab = "Sample Quantiles",
       plot.it = TRUE, datax = FALSE)
qqline(Reg_XY_data$X_TSM)

qqnorm(Reg_XY_data$Y_2330, main = "台灣台積電股價報酬率 Q-Q Plot",
       xlab = "Theoretical Quantiles", ylab = "Sample Quantiles",
       plot.it = TRUE, datax = FALSE)
qqline(Reg_XY_data$Y_2330)
...
#------------------------------------------
#補充:
origin.lm <- lm(X_TSM~Y_2330, data = Reg_XY_data)
summary(origin.lm)

#殘差獨立性檢定
library(car)
durbinWatsonTest(origin.lm)
